        ...
        <div class="grid gap-8 md:grid-cols-2">
            <div class="animate-on-scroll surface-card p-8" data-animate="slideLeft">
                <div class="mb-6 flex items-center gap-3">
                    ...
                    <h3 class="text-2xl font-bold text-zinc-900">{{ $research['title'] }}</h3>
                </div>
                ...
                <div class="grid grid-cols-2 gap-4">
                    @foreach ($research['highlights'] as $item)
                        <div class="inner-panel p-4">
                            <x-portfolio.icon :name="$item['icon']" class="mb-2 h-5 w-5 text-zinc-500" />
                            <div class="mb-1 text-xs text-zinc-500">{{ $item['label'] }}</div>
                            <div class="text-sm font-semibold text-zinc-800">{{ $item['value'] }}</div>
                        </div>
                    @endforeach
                </div>
            </div>
                    ...
                    <div class="space-y-3">
                        @foreach ($research['pipeline'] as $step)
                            <div class="inner-panel flex items-center gap-3 p-3">
                                <div class="flex h-6 w-6 items-center justify-center rounded-full border border-portfolio-border bg-portfolio-bg-soft text-xs font-medium text-zinc-600">
                                    {{ $loop->iteration }}
                                </div>
                                <span class="text-sm text-zinc-600">{{ $step }}</span>
                            </div>
                        @endforeach
                    </div>
                </div>
                ...
                <div class="animate-on-scroll surface-card p-6" data-animate="fadeInUp" data-delay="0.2">
                    <div class="grid grid-cols-3 gap-4 text-center">
                        @foreach ($research['stats'] as $stat)
                            <div>
                                <div class="mb-1 text-2xl font-bold text-zinc-900">{{ $stat['value'] }}</div>
